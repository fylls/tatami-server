...
if (!SECRET_KEY) console.log("SECRET_KEY is missing")
...
router.post("/bootcamp", async (req: Request, res: Response) => {
	...
	// search for user
	let user = await Customer.findOne({ email: mail })
	...
	// get user (both new or old)
	user = await Customer.findOne({ email: mail })
